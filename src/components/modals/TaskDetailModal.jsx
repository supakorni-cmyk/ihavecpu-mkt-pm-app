...
import { TAG_COLORS, formatDate } from '../../utils/constants';
...
export default function TaskDetailModal({ task, onClose, onEdit, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]" onClick={onClose}>
      <div 
        className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200" 
      ...
      >
        ...
        <div className="px-8 pt-6 pb-4 overflow-y-auto custom-scrollbar flex-1">
            ...
            <div className="mb-6 mt-2">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-500 font-medium">
                    <div className="flex items-center gap-1.5">
                        ...
                        {task.startDate ? new Date(task.startDate).toLocaleDateString('en-GB') : (task.deadline ? formatDate(task.deadline) : 'No Date')}
                    </div>
                    {task.startTime && (
                        <div className="flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                {task.reference && (
                    <button 
                        onClick={() => openLink(task.reference)}
                        className="flex items-center gap-3 p-3 rounded-xl border border-blue-100 bg-blue-50/50 hover:bg-blue-100 transition text-left group"
                    >
                        <div className="bg-blue-100 p-2 rounded-lg text-blue-600 group-hover:bg-white transition"><LinkIcon size={18}/></div>
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-blue-800 uppercase tracking-wide">Reference / Script</p>
                            <p className="text-xs text-blue-600 truncate underline decoration-blue-300">Click to Open</p>
                        </div>
                ...
                {task.finalFile && (
                    <button 
                        onClick={() => openLink(task.finalFile)}
                        className="flex items-center gap-3 p-3 rounded-xl border border-green-100 bg-green-50/50 hover:bg-green-100 transition text-left group"
                    >
                        <div className="bg-green-100 p-2 rounded-lg text-green-600 group-hover:bg-white transition"><FileText size={18}/></div>
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-green-800 uppercase tracking-wide">Final Work</p>
                            <p className="text-xs text-green-600 truncate underline decoration-green-300">Click to Download</p>
                        </div>
            ...
            {reqs.length > 0 && (
                <div className="mb-4">
                    <h4 className="font-bold text-gray-800 text-sm mb-3 flex items-center gap-2">
                        <CheckSquare size={18} className="text-gray-400"/> Requirements ({completedReqs}/{reqs.length})
                    </h4>
                    <div className="space-y-2">
                        {reqs.map(req => (
                            <div key={req.id} className="flex items-start gap-3 p-2 rounded-lg bg-gray-50/50">
                                ...
                                `}>
                                    {req.isDone && <div className="w-1.5 h-2.5 border-b-2 border-r-2 border-white rotate-45 mb-0.5"></div>}
                                </div>
                                <span className={`text-sm ${req.isDone ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                                    {req.title || req.text}
                                </span>
                            </div>
                        ))}
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
            ...
            <button 
                onClick={onEdit}
                className="flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-bold text-white bg-gray-900 hover:bg-black shadow-lg transition"
            >
