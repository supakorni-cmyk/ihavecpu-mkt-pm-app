        ...
        return (
            <div className="flex-1 flex flex-col h-full bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="grid grid-cols-7 border-b border-gray-100 bg-gray-50">
                    {dayNames.map((day, i) => <div key={day} className={`py-3 text-center text-xs font-bold uppercase tracking-wider ${i === 0 || i === 6 ? 'text-red-400' : 'text-gray-500'}`}>{day}</div>)}
                </div>
                    ...
                    {weekDays.map((dateObj, i) => {
                        ...
                        return (
                            <div key={i} onClick={() => handleDateClick(dateObj)} className={`flex flex-col h-full cursor-pointer hover:bg-gray-50 transition-colors ${isCurrent ? 'bg-blue-50/30' : 'bg-white'}`}>
                                <div className={`p-3 text-center border-b border-gray-100 ${isCurrent ? 'bg-blue-50' : 'bg-gray-50'}`}><p className={`text-xs font-bold uppercase mb-1 ${i === 0 || i === 6 ? 'text-red-400' : 'text-gray-500'}`}>{dayNames[dateObj.getDay()]}</p><div className={`mx-auto w-8 h-8 flex items-center justify-center rounded-full text-lg font-black ${isCurrent ? 'bg-blue-600 text-white shadow' : 'text-gray-800'}`}>{dateObj.getDate()}</div></div>
        ...
        return (
            <div className="flex-1 bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col max-w-4xl mx-auto w-full relative">
                <div className="p-6 border-b border-gray-100 bg-gray-50 flex items-center gap-4">
                    <div className="w-16 h-16 bg-blue-600 rounded-2xl flex flex-col items-center justify-center text-white shadow-lg"><span className="text-xs font-bold uppercase">{dayNames[currentDate.getDay()]}</span><span className="text-3xl font-black leading-none">{currentDate.getDate()}</span></div>
    ...
    return (
        <div className="flex flex-col h-full w-full bg-gray-50 font-sans relative">
            <header className="px-8 py-5 border-b border-gray-200 bg-white flex flex-col md:flex-row justify-between items-center shadow-sm z-20 gap-4">
                <div className="flex items-center gap-4"><h2 className="text-3xl font-black text-gray-800 tracking-tight flex items-center gap-3">{viewMode === 'day' ? `${currentDate.getDate()} ${monthNames[month]}` : monthNames[month]} <span className="text-gray-400 font-light">{year}</span></h2><div className="flex items-center bg-gray-100 rounded-lg p-1 gap-1"><button onClick={() => navigate('prev')} className="p-1.5 hover:bg-white hover:shadow-sm rounded-md transition text-gray-600"><ChevronLeft size={18} /></button><button onClick={goToToday} className="px-3 py-1 text-xs font-bold text-gray-600 hover:bg-white hover:shadow-sm rounded-md transition">Today</button><button onClick={() => navigate('next')} className="p-1.5 hover:bg-white hover:shadow-sm rounded-md transition text-gray-600"><ChevronRight size={18} /></button></div></div>
                <div className="flex items-center gap-3"><button onClick={handleAiBriefing} disabled={isGenerating} className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold transition shadow-sm ${isGenerating ? 'bg-indigo-50 text-indigo-400 cursor-wait' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}><Sparkles size={14} className={isGenerating ? "animate-spin" : ""} />{isGenerating ? "Analyzing..." : "AI Daily Brief"}</button><div className="flex bg-gray-100 p-1 rounded-lg">{['month', 'week', 'day'].map((mode) => (<button key={mode} onClick={() => setViewMode(mode)} className={`px-4 py-1.5 rounded-md text-xs font-bold capitalize transition-all ${viewMode === mode ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{mode}</button>))}</div></div>
            </header>
            {aiSummary && (
                <div className="mx-6 mt-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl relative animate-in slide-in-from-top-2"><button onClick={() => setAiSummary(null)} className="absolute top-2 right-2 text-indigo-300 hover:text-indigo-600 transition"><X size={16} /></button><div className="flex gap-3"><div className="mt-1 bg-indigo-100 p-2 rounded-full h-fit text-indigo-600"><Sparkles size={20} /></div><div><h4 className="text-sm font-bold text-indigo-900 mb-1">Morning Briefing</h4><p className="text-sm text-indigo-800 leading-relaxed whitespace-pre-line">{aiSummary}</p></div></div></div>
            )}
