...
const HomeView = ({ tasks, currentUser }) => {
    ...
    // Calculate Tag distribution for the "Workload" chart
                            ...
                            <div className="flex items-end justify-between h-64 gap-4">
                                {[
                                    ...
                                    <div key={stat.label} className="flex flex-col items-center gap-2 flex-1 h-full justify-end group">
                                        <div className="font-bold text-gray-800 mb-1 opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0">
                                            {stat.count}
                                        </div>
                                        <div 
                                        ...
                                        ></div>
                                        <div className="text-xs font-bold text-gray-400 uppercase text-center mt-2">
                                            {stat.label}
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-5">
                                {Object.keys(TAG_COLORS).map((tag) => { 
                                    ...
                                    // Extract just the background color class from the string
...
// Simple internal sub-component for the top cards to keep code clean
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between h-32 hover:shadow-md transition">
            <div className="flex justify-between items-start">
                <div className={`${activeColor.bg} ${activeColor.text} p-2 rounded-lg`}>
                    <Icon size={24} />
                </div>
