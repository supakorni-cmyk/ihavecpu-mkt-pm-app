  ...
  return (
    <div className="flex flex-col h-full w-full">
      <header className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white/80 backdrop-blur-md z-10">
        ...
        <button onClick={onAddTaskClick} className="flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-full font-medium hover:bg-gray-800 transition shadow-lg shadow-gray-200">
              ...
              <div key={col.id} className="flex-1 min-w-[300px] flex flex-col h-full">
                <div className="flex items-center justify-between mb-4 px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full text-xs font-bold">{columnTasks.length}</span>
                  </div>
                ...
                <div className={`flex-1 rounded-2xl p-2 ${col.color} overflow-y-auto custom-scrollbar`}>
                  <div className="flex flex-col gap-3 pb-2">
                    {columnTasks.map(task => (
                      <TaskCard key={task.id} task={task} columnId={col.id} onClick={onTaskClick} onDelete={onDeleteTask} onMove={onMoveTask} />
                    ))}
                  </div>
                </div>
              </div>
  ...
  return (
    <div onClick={() => onClick(task.id)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all group relative cursor-pointer">
      <div className="flex justify-between items-start mb-3">
        <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wide uppercase ${TAG_COLORS[task.tag] || 'bg-gray-100 text-gray-500'}`}>{task.tag}</span>
        <button onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1"><Trash2 size={14} /></button>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-50">
        <div className="flex items-center gap-1.5 text-gray-400 text-xs font-medium">
          <Clock size={12} />
          {/* CHANGED: Show start date shorthand if available, or just deadline */}
          <span>{task.startDate ? `${new Date(task.startDate).getDate()}/${new Date(task.startDate).getMonth()+1} - ` : ''}{formatDate(task.deadline)}</span>
        </div>
        
        <div className="flex gap-1" onClick={e => e.stopPropagation()}>
          {columnId !== 'todo' && (
            <button onClick={() => onMove(task.id, task.status || 'todo', 'prev')} className="p-1 hover:bg-gray-100 rounded text-gray-400 hover:text-blue-600 transition"><ArrowLeft size={14} /></button>
          )}
          {columnId !== 'done' && (
            <button onClick={() => onMove(task.id, task.status || 'todo', 'next')} className="p-1 hover:bg-gray-100 rounded text-gray-400 hover:text-blue-600 transition"><ArrowRight size={14} /></button>
          )}
        </div>
      </div>
    </div>
