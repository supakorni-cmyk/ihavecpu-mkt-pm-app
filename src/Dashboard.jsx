...
export default function Dashboard() {
  ...
  // requirements is now an array of strings temporarily for input
  const [newTask, setNewTask] = useState({
    ...
    deadline: '',
    description: '',
    requirements: [], // Array of objects: { id, text, isDone, tableData }
    reference: '',
    link: '',
    imageUrl: '',
    fileUrl: ''
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Update selectedTask live
      if (selectedTask) {
         const currentSelected = taskData.find(t => t.id === selectedTask.id);
         if (currentSelected) {
             setSelectedTask(currentSelected);
             // Also update active requirement view if open
             if (activeRequirement) {
                 // We need to find the specific requirement inside the updated task
                 // We assume requirements have stable IDs or we match by index if migrating
  ...
  // Add a requirement line item in the Add Task Modal
  const addRequirementLine = () => {
      ...
      const newReq = {
          ...
          text: tempReqInput,
          isDone: false,
          tableData: [] // Initialize empty table for this requirement
      };
  ...
  // Toggle Checkbox for a Requirement
  const toggleRequirement = async (taskId, reqId, currentRequirements) => {
      let updatedReqs = [];
      
      // Handle legacy string data (Migration)
      if (typeof currentRequirements === 'string') {
          ...
          // Find the one we clicked (by index since we just generated IDs) - Logic approximation
          // Better to just migrate first. But here we assume user clicked the mapped view.
          // Since we can't easily match ID on legacy string render, we skip direct toggle on legacy
          // and force a migration first or rely on the Edit view.
          // However, for smooth UX, let's assume the View mapped it.
      } else {
          updatedReqs = currentRequirements.map(r => 
  ...
  const deleteTableRow = async (taskId, reqId, rowId) => {
      const updatedReqs = selectedTask.requirements.map(r => {
          if (r.id === reqId) {
              return { 
                  ...r, 
                  tableData: r.tableData.filter(row => row.id !== rowId) 
              };
          }
          return r;
      });
  ...
  const exportToCSV = (req) => {
      ...
      const headers = ["Column 1", "Column 2", "Column 3", "Notes"];
  ...
  // --- HELPER: Migrate Legacy Data Helper ---
  // Returns a safe array of requirement objects even if DB has a string
  ...
  // ... (Keep existing moveTask, deleteTask, logout, etc.)
  const moveTask = async (e, taskId, currentStatus, direction) => { e.stopPropagation(); const statusOrder = ['todo', 'inprogress', 'review', 'done']; const currentIndex = statusOrder.indexOf(currentStatus); if (currentIndex === -1) { await updateDoc(doc(db, 'tasks', taskId), { status: 'todo' }); return; } let nextIndex = direction === 'next' ? currentIndex + 1 : currentIndex - 1; if (nextIndex >= 0 && nextIndex < statusOrder.length) { await updateDoc(doc(db, 'tasks', taskId), { status: statusOrder[nextIndex] }); } };
  const deleteTask = async (e, id) => { e.stopPropagation(); if (confirm("Delete task?")) { await deleteDoc(doc(db, 'tasks', id)); if (selectedTask?.id === id) setSelectedTask(null); } };
              ...
              <div className="bg-white w-full max-w-6xl h-[85vh] rounded-xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
                  ...
                  <div className="bg-green-600 px-6 py-4 flex justify-between items-center text-white shrink-0">
                      ...
                      <div className="flex gap-3">
                          <button onClick={() => exportToCSV(requirement)} className="bg-white text-green-700 px-4 py-2 rounded-lg font-bold text-sm hover:bg-green-50 transition flex items-center gap-2"><Download size={16} /> Export CSV</button>
                          <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-full text-white"><X size={24} /></button>
                      </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-auto bg-gray-50 p-6">
                      <div className="bg-white border border-gray-300 shadow-sm min-w-[800px]">
                          ...
                          <div className="flex border-b border-gray-300 bg-gray-100 text-gray-500 font-bold text-xs uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                              ...
                              <div className="flex-1 p-3 border-r border-gray-300">Item / Name</div>
                              <div className="flex-1 p-3 border-r border-gray-300">Description / Spec</div>
                              <div className="w-32 p-3 border-r border-gray-300">Status / Qty</div>
                              <div className="flex-1 p-3 border-r border-gray-300">Notes</div>
                          ...
                          {(requirement.tableData || []).map((row, idx) => (
                              <div key={row.id} className="flex border-b border-gray-200 hover:bg-blue-50/30 transition-colors">
                                  ...
                                  <div className="flex-1 p-3 border-r border-gray-200 text-sm">{row.col1}</div>
                                  <div className="flex-1 p-3 border-r border-gray-200 text-sm">{row.col2}</div>
                                  <div className="w-32 p-3 border-r border-gray-200 text-sm">{row.col3}</div>
                                  <div className="flex-1 p-3 border-r border-gray-200 text-sm text-gray-500 italic">{row.notes}</div>
                                  <div className="w-12 p-3 flex items-center justify-center">
                                      <button onClick={() => deleteTableRow(task.id, requirement.id, row.id)} className="text-gray-300 hover:text-red-500"><Trash2 size={14} /></button>
                                  </div>
                              </div>
                          ))}
                          ...
                          <div className="flex border-b border-gray-200 bg-yellow-50/50">
                              ...
                              <div className="w-12 p-2 flex items-center justify-center">
                                  <button onClick={handleAddRow} className="bg-green-600 text-white p-1 rounded hover:bg-green-700"><Plus size={16} /></button>
                              </div>
                          </div>
                      </div>
                  </div>
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50 font-sans overflow-hidden">
      ...
      <aside className="w-20 md:w-64 bg-white border-r border-gray-200 flex flex-col justify-between flex-shrink-0 z-20 print:hidden">
        <div>
            <div className="p-6 flex items-center gap-3 mb-6">
                ...
                <div className="hidden md:block"><h1 className="text-lg font-bold text-gray-900 leading-none">iHAVECPU</h1><span className="text-xs text-blue-600 font-bold tracking-wider">WORKSPACE</span></div>
            </div>
            <nav className="px-3 space-y-2">
                {[{ id: 'home', icon: Home, label: 'Home' }, { id: 'board', icon: Layout, label: 'Board' }, { id: 'calendar', icon: CalendarIcon, label: 'Calendar' }, { id: 'report', icon: Presentation, label: 'Report' }, { id: 'album', icon: ImageIcon, label: 'Photo Album' }, { id: 'selfheal', icon: Heart, label: 'Self Heal' }].map(item => (
                    <button key={item.id} onClick={() => setCurrentView(item.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${currentView === item.id ? 'bg-blue-50 text-blue-600 font-bold' : 'text-gray-500 hover:bg-gray-50'}`}>
        ...
        <div className="p-4 border-t border-gray-100">
             <div className="flex items-center gap-3 px-4 py-3 mb-2">
                <div className="h-8 w-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-white text-xs font-bold shadow-md shrink-0">{currentUser?.email?.charAt(0).toUpperCase()}</div>
                <div className="hidden md:block overflow-hidden"><p className="text-sm font-bold text-gray-700 truncate">{currentUser?.email?.split('@')[0]}</p><p className="text-[10px] text-gray-400">Admin</p></div>
             </div>
             <button onClick={handleLogout} className="w-full flex items-center gap-2 text-gray-400 hover:text-red-500 px-4 py-2 rounded-lg hover:bg-red-50 transition text-sm"><LogOut size={16} /> <span className="hidden md:inline">Log out</span></button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full w-full overflow-hidden bg-white relative">
        ...
        {currentView !== 'board' && <div className="flex items-center justify-center h-full text-gray-400">View: {currentView} (See previous code for full implementation)</div>}
        ...
        {currentView === 'board' && (
            <div className="flex flex-col h-full w-full">
                <header className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white/80 backdrop-blur-md z-10">
                     ...
                     <button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-full font-medium hover:bg-gray-800 transition shadow-lg shadow-gray-200"><Plus size={18} /> New Task</button>
                </header>
                <div className="flex-1 overflow-x-auto overflow-y-hidden px-6 pb-4 pt-6">
                    ...
                    {columns.map(col => (
                        ...
                        <div className="flex items-center justify-between mb-4 px-1"><div className="flex items-center gap-2"><h3 className="text-gray-600 font-bold text-sm uppercase tracking-wider">{col.title}</h3><span className="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full text-xs font-bold">{getTasksByStatus(col.id).length}</span></div><MoreHorizontal size={16} className="text-gray-300" /></div>
                        <div className={`flex-1 rounded-2xl p-2 ${col.color} overflow-y-auto custom-scrollbar`}>
                            <div className="flex flex-col gap-3 pb-2">
                                {getTasksByStatus(col.id).map(task => (
                                    <div key={task.id} onClick={() => { setSelectedTask(task); setIsEditing(false); }} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all group relative cursor-pointer">
                                        <div className="flex justify-between items-start mb-3"><span className={`px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wide uppercase ${tagColors[task.tag] || 'bg-gray-100 text-gray-500'}`}>{task.tag}</span><button onClick={(e) => deleteTask(e, task.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-1"><Trash2 size={14} /></button></div>
                                        <h4 className="text-gray-800 font-semibold text-sm mb-4 leading-relaxed line-clamp-2">{task.title}</h4>
                                        <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                                            <div className="flex items-center gap-1.5 text-gray-400 text-xs font-medium"><Clock size={12} /><span>{formatDate(task.deadline)}</span></div>
                                            <div className="flex gap-1">{col.id !== 'todo' && <button onClick={(e) => moveTask(e, task.id, task.status || 'todo', 'prev')} className="p-1 hover:bg-gray-100 rounded text-gray-400 hover:text-blue-600"><ArrowLeft size={14} /></button>}{col.id !== 'done' && <button onClick={(e) => moveTask(e, task.id, task.status || 'todo', 'next')} className="p-1 hover:bg-gray-100 rounded text-gray-400 hover:text-blue-600"><ArrowRight size={14} /></button>}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
      ...
      {/* --- ADD TASK MODAL (UPDATED) --- */}
      {isAddModalOpen && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 md:p-8">
                    <div className="flex justify-between items-center mb-6"><h3 className="text-2xl font-bold text-gray-800">Create New Task</h3><button onClick={() => setIsAddModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full text-gray-500"><X size={24} /></button></div>
                    <form onSubmit={handleAddTask} className="flex flex-col gap-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input autoFocus type="text" className="w-full border-gray-200 bg-gray-50 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 font-medium" placeholder="Task Title" value={newTask.title} onChange={e => setNewTask({...newTask, title: e.target.value})} /><select className="w-full border-gray-200 bg-gray-50 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800" value={newTask.tag} onChange={e => setNewTask({...newTask, tag: e.target.value})}><option value="Planning">Planning</option><option value="Project">Project</option><option value="Product Review">Product Review</option><option value="Event">Event</option><option value="Guest Speaker">Guest Speaker</option></select></div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input type="date" className="w-full border-2 border-blue-200 bg-blue-50 rounded-lg px-4 py-3 font-bold" value={newTask.deadline} onChange={e => setNewTask({...newTask, deadline: e.target.value})} /></div>
                        
                        {/* New Requirement Input Logic */}
                        <div>
                            ...
                            <div className="flex gap-2 mb-2">
                                <input 
                                    ...
                                    className="flex-1 border-gray-200 bg-gray-50 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    value={tempReqInput}
                                    onChange={e => setTempReqInput(e.target.value)}
                                    onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addRequirementLine())}
                                />
                ...
                <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex-1">
                            {!isEditing ? (
                                <>
                                    <div className="flex items-center gap-3 mb-3"><span className={`px-3 py-1 rounded-md text-xs font-bold tracking-wide uppercase ${tagColors[selectedTask.tag] || 'bg-gray-100'}`}>{selectedTask.tag}</span><span className="text-gray-400 text-xs flex items-center gap-1"><Clock size={12} /> Created {formatDate(selectedTask.createdAt?.seconds ? new Date(selectedTask.createdAt.seconds * 1000) : selectedTask.createdAt)}</span></div>
                        ...
                        <div className="flex gap-2 ml-4">
                            {!isEditing ? <button onClick={startEditing} className="p-2 hover:bg-blue-50 text-blue-600 rounded-full transition"><Edit2 size={20} /></button> : <button onClick={() => setIsEditing(false)} className="px-4 py-2 text-sm font-bold text-gray-500 hover:bg-gray-100 rounded-lg">Cancel</button>}
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                ...
                                <div className="space-y-3 ml-1">
                                    {getSafeRequirements(selectedTask).length === 0 && <p className="text-gray-400 italic text-sm">No requirements added.</p>}
                                    {getSafeRequirements(selectedTask).map((req) => (
                                        <div key={req.id} className="flex items-start gap-3 group">
                                            <input 
                                                type="checkbox" 
                                                checked={req.isDone} 
                                                onChange={() => toggleRequirement(selectedTask.id, req.id, selectedTask.requirements)}
                                            ...
                                            <div className="flex-1">
                                                <span 
                                                    ...
                                                    title="Click to open detailed table"
                                                >
                                                    {req.text}
                                                </span>
                                                {/* Mini badge if table has data */}
                                                {req.tableData && req.tableData.length > 0 && <span className="ml-2 text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-bold">{req.tableData.length} items</span>}
                                            </div>
                                            <button onClick={() => setActiveRequirement(req)} className="text-blue-500 opacity-0 group-hover:opacity-100 transition text-xs font-bold hover:underline">Open Table</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            {/* Description */}
                            <div><h4 className="flex items-center gap-2 text-lg font-bold text-gray-800 mb-3"><AlignLeft size={20} className="text-gray-400" /> Description</h4><p className="text-gray-600 leading-relaxed whitespace-pre-wrap pl-7">{selectedTask.description || <span className="italic text-gray-400">No description provided.</span>}</p></div>
                        </div>
                    ) : (
                        <form onSubmit={handleUpdateTask} className="flex flex-col gap-6">
                             {/* Simplified Edit Mode for title/desc/date only to avoid complex state management on the fly */}
                             <div className="p-4 bg-yellow-50 text-yellow-700 text-sm rounded-lg">To edit Requirements or their Tables, please use the View mode interaction.</div>
                             <textarea className="w-full border-gray-200 bg-gray-50 rounded-lg px-4 py-3 min-h-[100px]" value={editedTask.description} onChange={e => setEditedTask({...editedTask, description: e.target.value})} placeholder="Description..." />
      ...
      {/* --- REQUIREMENT SHEET MODAL (THE SEPARATE TABLE) --- */}
