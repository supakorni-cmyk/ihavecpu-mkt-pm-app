...
import { 
  ...
  Youtube, Facebook, User, AtSign, Zap, Settings, DollarSign, TrendingUp, TrendingDown, BarChart3, Wallet
} from 'lucide-react';
...
const SimpleLineChart = ({ data, color = "#3b82f6" }) => {
    ...
    const height = 150;
    const width = 1000; // Arbitrary high res width for SVG viewBox
    const padding = 10;
    ...
    // Fill area path (close the loop at the bottom)
...
const BudgetRecorderView = ({ transactions, onAdd, onDelete }) => {
    ...
    const handleAddTransaction = (e) => {
        e.preventDefault();
        onAdd({ ...newTransaction, type: activeTab === 'overview' ? 'income' : activeTab, createdAt: new Date() });
        setIsAddOpen(false);
        setNewTransaction({ type: 'income', date: new Date().toISOString().split('T')[0], brand: '', category: BUDGET_CATEGORIES[0], description: '', amount: '', company: '', invoice: '', paymentDate: '', status: 'Pending', remark: '' });
    };
    ...
    sortedTransactions.forEach(t => {
        ...
        // Uncomment below if income should increase budget
        // else if(t.type === 'income') runningBalance += (parseFloat(t.amount) || 0);
        ...
        <div className="flex flex-col h-full bg-gray-50 font-sans">
            <header className="px-8 py-5 border-b border-gray-200 bg-white shadow-sm z-10 flex justify-between items-center">
                ...
                <div className="flex items-center gap-6">
                    {activeTab !== 'overview' && (
                        <div className="text-right"><p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total {activeTab}</p><p className={`text-2xl font-black ${activeTab === 'income' ? 'text-green-600' : 'text-red-600'}`}>฿{tabTotal.toLocaleString()}</p></div>
                    )}
                ...
                <div className="px-8 pt-6 pb-0 flex gap-1 border-b border-gray-200 bg-gray-50">
                    {['overview', 'income', 'spending'].map(tab => (
                        <button 
                            ...
                            className={`px-8 py-3 font-bold text-sm rounded-t-xl transition-all capitalize relative ${activeTab === tab ? 'bg-white shadow-sm border border-b-0 border-gray-200' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-700'} ${activeTab === tab && tab === 'income' ? 'text-green-600' : activeTab === tab && tab === 'spending' ? 'text-red-600' : activeTab === tab ? 'text-blue-600' : ''}`}
                        >
                            {tab}
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {filteredTransactions.map((t) => (
                                        <tr key={t.id} className="hover:bg-blue-50/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-center"><button onClick={() => onDelete(t.id)} className="text-gray-300 hover:text-red-500 transition opacity-0 group-hover:opacity-100 p-1 rounded-md hover:bg-red-50"><Trash2 size={16} /></button></td>
                                        </tr>
                                    ))}
                                    {filteredTransactions.length === 0 && (<tr><td colSpan="11" className="px-6 py-12 text-center text-gray-400 font-medium">No records found for this view.</td></tr>)}
                                </tbody>
                            </table>
                        </div>
                    )}
                ...
                <div className="fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-4xl p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
                        ...
                        <form onSubmit={handleAddTransaction} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                             {/* Form content same as before, simplified for brevity but fully functional in logic above */}
                            <div className="space-y-6">
                                ...
                                <div><label className="block text-xs font-bold text-gray-500 uppercase mb-2">Brand</label><input required type="text" placeholder="e.g. Intel" className="w-full border border-gray-200 rounded-lg p-3 outline-none focus:ring-2 focus:ring-blue-500 transition" value={newTransaction.brand} onChange={e => setNewTransaction({...newTransaction, brand: e.target.value})} /></div>
                            ...
                            <div className="md:col-span-2 pt-6 border-t border-gray-100 flex justify-end gap-3"><button type="button" onClick={() => setIsAddOpen(false)} className="px-6 py-3 rounded-lg font-bold text-gray-500 hover:bg-gray-100 transition">Cancel</button><button type="submit" className="px-8 py-3 rounded-lg font-bold text-white shadow-lg transition transform hover:scale-105 bg-gray-900 hover:bg-black">Save Record</button></div>
                        </form>
                    </div>
                </div>
            )}
            ...
            <div className="p-6 md:p-10 overflow-y-auto flex-1">
                <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100"><h3 className="text-lg font-bold text-gray-800 mb-6">Workload</h3><div className="space-y-5">{Object.keys(TAG_COLORS).map((tag) => { const count = tasks.filter(t => t.tag === tag).length; return (<div key={tag}><div className="flex justify-between text-sm font-bold mb-2"><span className="text-gray-600">{tag}</span><span className="text-gray-400">{count} Tasks</span></div><div className="h-3 w-full bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-500 ${(TAG_COLORS[tag] || 'bg-gray-200').split(' ')[0]}`} style={{ width: `${(count / maxTagCount) * 100}%` }}></div></div></div>) })}</div></div>
                    </div>
                </div>
            </div>
        </div>
